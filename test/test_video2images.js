...
async function main() {
    let ffmpegNode = new FFmpegNode(rtsp_addr, false);
    let begin = new Date();
    ...
    try {
        for (let i = 0; i < 10; i++)
            await ffmpegNode.readRgbStream(2, async (buffer) => {
                for (var start = Date.now(); Date.now() - start <= 300;) { }
                let t1 = new Date();
                console.info(buffer);
                console.info(t1.getTime() - begin.getTime());
                let name = dir + "/tmp/images/buffer-" + t1.getHours() + "-" + t1.getMinutes() + "-" + t1.getSeconds() + "-" + i + ".rgb";
                console.info(name);
                await fs.writeFileSync(name, buffer, (err) => {
                    if (err) console.error(err);
                });
